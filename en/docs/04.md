...
In ABCI System, 1.6 TB NVMe storage is installed into each compute node and 1.8 TB SATA 3.0 storage is installed into each memory intensive node.
This local storage is available without specifying any additional options.
Note that the amount of the local storage you can use is determined by "Resource type".
For more detail on "Resource type", please refer to [Job Executing Resource](03.md#job-executing-resource).
...
!!! warning
    The file stored under local storage is removed when job finished.
    The required files need to be moved to Home area or Group area in job script using `cp` command.

### 4.3.1. Using as a BeeOND storage {#using-as-a-beeond-storage}

In ABCI System, using the local storages of job assigned compute nodes as the data storage, the distributed shared file system BeeGFS On Demand(BeeOND) is available on demand.
The BeeOND area is created and deleted for each job and can be accessed from /beeond.
When using BeeOND, you need to submit job with `-l USE_BEEOND=1` option.
